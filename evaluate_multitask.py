...
def sorted_confidence_save(select):
    ...
    for img_name in high_confidence: # 사진 저장
        shutil.copyfile(f'./data/airway/train/{img_name}', f'./confidence/{args.kernel_type}_confidence/{args.kernel_type}_high_confidence_image/{img_name}')
        ...
        if args.side_task_type:
            for (data, target, target2, patients_id, image_name) in tqdm(loader):
                ...
                if args.use_meta:
                    ...
                    data, target, target2, patients_id = data.to(device), target.to(device), target2.to(
                        device), patients_id.to(device)
                    logits = torch.zeros((data.shape[0], args.out_dim)).to(device)
...
def main():
    ...
    for fold in folds:
        ...
        ####################################################
        '''
        this_LOGITS, this_PROBS, this_TARGETS, this_PATIENT_ID, this_IMAGE_NAME, val_loss, acc, f1, recall, precision, auc, optimal_threshold = val_epoch(model, valid_loader, target_idx, fold, is_ext=df_valid['is_ext'].values, n_test=8, get_output=True)
