...
def parse_args():
    ...
    parser.add_argument('--select', type=int, default=100) # sorting된 confidence csv에서 가져올 이미지 개수
...
def confidence_score(PROBS, TARGETS, PATIENT_ID, IMAGE_NAME):
    ...
    confidence_dict['correct_prob'] = (confidence_dict['prob'] > args.thresholds) * confidence_dict['target'] * confidence_dict['prob']
    ...
    confidence_csv.to_csv(f'./confidence/{args.kernel_type}_pid_mean_confidence.csv')
...
def sorted_confidence_save(select):
    ...
    confidence_csv[:select].to_csv(f'./confidence/{args.kernel_type}_pid_mean_high_confidence.csv', index=False)
    high_confidence = confidence_csv[['image_name']][:select].squeeze().tolist()

    for img_name in high_confidence: # 사진 저장
        shutil.copyfile(f'./data/airway/train/{img_name}', f'./confidence/high_confidence_image/pid_mean_{img_name}')
...
def plot_roc_curve(fper, tper, fold, optimal_threshold):
    ...
    plt.savefig(f'./roc_curve_image_all_data/{args.kernel_type}_{fold}.png')
    plt.show()
...
def val_epoch(model, loader, target_idx, fold, is_ext=None, n_test=1, get_output=False):
    ...
    #     ConT.append(np.unique(TARGETS[res_list], axis=0))
    #     save_prob = PROBS[res_list]
    #     mean_prob = save_prob.mean(axis=0)
    #     ConP.append(mean_prob)
    ...
    return LOGITS, PROBS, TARGETS, PATIENT_ID, IMAGE_NAME, val_loss, acc
...
def main():
    ...
    for fold in folds:
        ...
        ####################################################
        '''
        this_LOGITS, this_PROBS, this_TARGETS, this_PATIENT_ID, this_IMAGE_NAME, val_loss, acc = val_epoch(model, valid_loader, target_idx, fold, is_ext=df_valid['is_ext'].values, n_test=8, get_output=True)
    ...
    # roc-curve
    fpr, tpr, threshold = roc_curve(np.concatenate(TARGETS), np.concatenate(PROBS)[:, target_idx])
    ...
    # f1-score
    f1 = f1_score(np.concatenate(TARGETS), np.concatenate(PROBS)[:, target_idx] > optimal_threshold)

    # recall-score
    recall = recall_score(np.concatenate(TARGETS), np.concatenate(PROBS)[:, target_idx] > optimal_threshold)

    # precision
    precision = precision_score(np.concatenate(TARGETS), np.concatenate(PROBS)[:, target_idx] > optimal_threshold)

    # auc
    auc = roc_auc_score(np.concatenate(TARGETS), np.concatenate(PROBS)[:, target_idx])


    # fold별 val_loss, acc, auc, recall,  출력
    ...
    print('auc: ', f'{auc:.4f}')
    print(classification_report(np.concatenate(TARGETS), np.concatenate(PROBS)[:, target_idx] > optimal_threshold))

        # print(classification_report(np.concatenate(TARGETS), np.concatenate(PROBS).argmax(1)))

    # 전체 fold 끝
    # 전체 데이터에 대한 confidence 구하고, confidence sum 기준으로 sorting
    confidence_score(PROBS, TARGETS, PATIENT_ID, IMAGE_NAME)
...
if __name__ == '__main__':
    ...
    os.makedirs('./confidence/high_confidence_image', exist_ok=True)
