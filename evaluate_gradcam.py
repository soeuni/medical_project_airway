...
def parse_args():
    ...
    parser.add_argument('--out-dim2', type=int, default=0)
...
def main():
    ...
    # 저장된 grad-cam image에 target, probs 글씨 넣기
    # high_confidence_csv = pd.read_csv(f'./confidence/{args.kernel_type}_pid_mean_high_confidence.csv')
    high_confidence_csv = pd.read_csv(f'./confidence/{args.kernel_type}_confidence/{args.kernel_type}_high_confidence.csv')[:100]

    #grad cam에 넣을 사진 경로
    # path = './confidence/high_confidence_image/'
    for idx in range(len(high_confidence_csv)):
        # image = cv2.imread(f'./confidence/high_confidence_image/{high_confidence_csv["image_name"][idx]}')
        image = cv2.imread(f'./data/airway/train/{high_confidence_csv["image_name"][idx]}')
        ...
        cv2.imwrite(f'./grad_cam/{args.kernel_type}_gradcam_image/{high_confidence_csv["correct_prob"][idx]:.2f}_{high_confidence_csv["image_name"][idx]}.png', cam_image)


    # 이미지에 target과 probs
    for idx in range(len(high_confidence_csv)):
        img = Image.open(f'./data/airway/train/{high_confidence_csv["image_name"][idx]}')
        draw = ImageDraw.Draw(img)

        # target = str(high_confidence_csv.loc[idx, 'target'].tolist())
        # prob = str(high_confidence_csv.loc[idx, 'prob'].tolist())
        # draw.text((13, 13), f"target: {target} \n prob: {prob}", font=ImageFont.truetype("C:\Windows\Fonts\gulim.ttc", 32), fill=(255, 255, 255))
        img.save(f'./grad_cam/{args.kernel_type}_gradcam_image/{high_confidence_csv["correct_prob"][idx]:.2f}_{high_confidence_csv["image_name"][idx]}')
...
if __name__ == '__main__':
    ...
    os.makedirs(f'./grad_cam/{args.kernel_type}_gradcam_image/', exist_ok=True)
    # os.makedirs('./gradcam_score_per_fold_image/', exist_ok=True)
